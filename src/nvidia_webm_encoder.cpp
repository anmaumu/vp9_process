...
void copy_plane(uint8_t *destination, int destination_stride,
                const uint8_t *source, int source_stride, uint32_t width,
                uint32_t height) {
...
mkvc_result convert_to_nv12(NvidiaWebmEncoder::Impl &state,
  ...
  case MKVC_PIXEL_FORMAT_NV12:
    ...
    copy_plane(nv_y, static_cast<int>(state.width), frame.planes[0],
               frame.strides[0], state.width, state.height);
    copy_plane(nv_uv, static_cast<int>(state.width), frame.planes[1],
               frame.strides[1], state.width, state.height / 2);
    return MKVC_OK;
  case MKVC_PIXEL_FORMAT_I420:
    ...
    converted = libyuv::I420ToNV12(
        ...
        frame.planes[2], frame.strides[2], nv_y, static_cast<int>(state.width),
        nv_uv, static_cast<int>(state.width), static_cast<int>(state.width),
  ...
  case MKVC_PIXEL_FORMAT_BGRA32: {
    ...
    if (frame.pixel_format == MKVC_PIXEL_FORMAT_BGR24)
      converted = libyuv::RGB24ToI420(
          frame.planes[0], frame.strides[0], y, state.width, u, state.width / 2,
          v, state.width / 2, state.width, state.height);
    else if (frame.pixel_format == MKVC_PIXEL_FORMAT_RGB24)
      converted = libyuv::RAWToI420(frame.planes[0], frame.strides[0], y,
                                    state.width, u, state.width / 2, v,
                                    state.width / 2, state.width, state.height);
    else
      converted = libyuv::ARGBToI420(
          frame.planes[0], frame.strides[0], y, state.width, u, state.width / 2,
          v, state.width / 2, state.width, state.height);
    if (converted == 0)
      converted = libyuv::I420ToNV12(y, state.width, u, state.width / 2, v,
                                     state.width / 2, nv_y, state.width, nv_uv,
...
void destroy_session(NvidiaWebmEncoder::Impl &state) {
  if (state.encoder != nullptr) {
    ...
    if (state.output != nullptr)
      (void)state.functions.nvEncDestroyBitstreamBuffer(state.encoder,
                                                        state.output);
...
bool initialize_session(NvidiaWebmEncoder::Impl &state, std::string &error) {
  ...
  if (state.functions.nvEncOpenEncodeSessionEx(&open, &state.encoder) !=
      NV_ENC_SUCCESS) {
  ...
  if (state.functions.nvEncInitializeEncoder(state.encoder, &init) !=
      NV_ENC_SUCCESS) {
  ...
  if (state.functions.nvEncCreateInputBuffer(state.encoder, &input) !=
      NV_ENC_SUCCESS) {
  ...
  if (state.functions.nvEncCreateBitstreamBuffer(state.encoder, &output) !=
      NV_ENC_SUCCESS) {
...
mkvc_result emit_packet(NvidiaWebmEncoder::Impl &state, std::string &error) {
  ...
  if (state.functions.nvEncLockBitstream(state.encoder, &lock) !=
      NV_ENC_SUCCESS) {
  ...
  if (!muxed) {
    error = "libwebm failed to mux NVENC AV1 packet";
...
std::unique_ptr<NvidiaWebmEncoder>
NvidiaWebmEncoder::create(const mkvc_encoder_config &config,
                          std::string &error) {
#if !defined(MKVC_HAS_NVIDIA)
  (void)config;
  ...
  state.context_create =
      state.cuda->symbol<tcuCtxCreate_v2 *>("cuCtxCreate_v2");
  state.context_destroy =
      state.cuda->symbol<tcuCtxDestroy_v2 *>("cuCtxDestroy_v2");
  const auto create_instance =
      state.nvenc->symbol<CreateInstance>("NvEncodeAPICreateInstance");
  ...
  if (!initialize_session(state, error))
    return nullptr;
  if (!state.writer.Open(config.output_path_utf8)) {
    error = "failed to open NVIDIA output";
    return nullptr;
  }
  state.writer_open = true;
  if (!state.segment.Init(&state.writer)) {
    error = "failed to initialize NVIDIA WebM muxer";
    return nullptr;
  }
  state.segment_initialized = true;
  state.segment.set_mode(mkvmuxer::Segment::kFile);
  state.track_number =
      state.segment.AddVideoTrack(config.width, config.height, 0);
  auto *track = static_cast<mkvmuxer::VideoTrack *>(
      state.segment.GetTrackByNumber(state.track_number));
...
mkvc_result NvidiaWebmEncoder::write(const mkvc_frame_view &frame,
  ...
  if (state.functions.nvEncLockInputBuffer(state.encoder, &lock) !=
      NV_ENC_SUCCESS) {
  ...
  const uint8_t *source_uv =
      source_y + static_cast<size_t>(state.width) * state.height;
  auto *destination = static_cast<uint8_t *>(lock.bufferDataPtr);
  copy_plane(destination, lock.pitch, source_y, state.width, state.width,
             state.height);
...
#else
  auto &state = *impl_;
  if (state.closed || state.frame_index == 0)
    return MKVC_OK;
  NV_ENC_PIC_PARAMS eos{};
  eos.version = NV_ENC_PIC_PARAMS_VER;
  eos.encodePicFlags = NV_ENC_PIC_FLAG_EOS;
  if (state.functions.nvEncEncodePicture(state.encoder, &eos) !=
      NV_ENC_SUCCESS) {

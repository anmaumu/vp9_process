...
namespace {
mkvc_result export_dlpack_impl(mkvc_gpu_frame* frame, uint32_t plane_index,
                               uint64_t consumer_stream, mkvc_gpu_dependency_callback callback,
                               void* callback_user_data, void** out_managed_tensor) {
    if (out_managed_tensor == nullptr)
        return fail(MKVC_ERROR_INVALID_ARGUMENT, "null DLPack output");
    *out_managed_tensor = nullptr;
    ...
    if (desc.device_id > static_cast<uint64_t>(std::numeric_limits<int32_t>::max()) ||
        desc.pitches[plane_index] > static_cast<uint64_t>(std::numeric_limits<int64_t>::max()) ||
        desc.plane_offsets[plane_index] > std::numeric_limits<uint64_t>::max() - native.handles[0])
    ...
    if (usm && consumer_stream != 0 && native.handles[3] != 0 && callback != nullptr) {
        try {
            result = callback(callback_user_data, native.handles[3], consumer_stream);
        } catch (...) {
            return fail(MKVC_ERROR_INTERNAL, "GPU dependency callback threw across the C boundary");
        }
        if (result != MKVC_OK)
...
#if defined(MKVC_HAS_NVIDIA)
        std::string error;
        result = mkvc::gpu::nvidia::cuda_stream_wait_event(native.handles[1], native.handles[3],
    ...
    state->managed.dl_tensor.device = {cuda ? kDLCUDA : kDLOneAPI,
                                       static_cast<int32_t>(desc.device_id)};
...
extern "C" mkvc_result mkvc_gpu_frame_export_dlpack(mkvc_gpu_frame* frame, uint32_t plane_index,
                                                    uint64_t consumer_stream,
                                                    void** out_managed_tensor) {
    return export_dlpack_impl(frame, plane_index, consumer_stream, nullptr, nullptr,
                              out_managed_tensor);
}
...
extern "C" mkvc_result mkvc_gpu_frame_export_dlpack_with_dependency(
    ...
    return export_dlpack_impl(frame, plane_index, consumer_stream, callback, user_data,
                              out_managed_tensor);
}

...
mkvc_result mkvc_cpu_frame_pool_acquire(
    mkvc_cpu_frame_pool* pool, uint32_t timeout_ms,
    mkvc_cpu_buffer** out_buffer) {
    last_error.clear();
    if (pool == nullptr || !pool->implementation || out_buffer == nullptr) {
        return fail(MKVC_ERROR_INVALID_ARGUMENT,
                    "invalid native CPU frame pool acquire");
    }
    *out_buffer = nullptr;
    try {
        ...
        const mkvc_result result = pool->implementation->acquire(
            timeout_ms, handle->implementation, error);
        if (result != MKVC_OK) {
...
mkvc_result mkvc_cpu_buffer_get_desc(
    const mkvc_cpu_buffer* buffer, mkvc_cpu_buffer_desc* out_desc) {
    last_error.clear();
    if (buffer == nullptr || !buffer->implementation || out_desc == nullptr ||
        out_desc->struct_size < sizeof(*out_desc) ||
        out_desc->struct_version != 1) {
        return fail(MKVC_ERROR_INVALID_ARGUMENT,
                    "invalid native CPU buffer descriptor output");
    }
...
mkvc_result mkvc_cpu_buffer_get_view(
    const mkvc_cpu_buffer* buffer, mkvc_mutable_frame_view* out_view) {
    last_error.clear();
    if (buffer == nullptr || !buffer->implementation || out_view == nullptr ||
        out_view->struct_size < sizeof(*out_view) ||
        out_view->struct_version != 1) {
        return fail(MKVC_ERROR_INVALID_ARGUMENT,
                    "invalid native CPU buffer view output");
    }
    try {
        mkvc_mutable_frame_view value{};
        std::string error;
        const mkvc_result result =
            buffer->implementation->get_view(value, error);
        if (result != MKVC_OK) return fail(result, std::move(error));
        *out_view = value;
        return MKVC_OK;
    } catch (const std::exception& exception) {
...
mkvc_result mkvc_encoder_submit_cpu_buffer(
    mkvc_encoder* encoder, const mkvc_cpu_buffer* buffer, int64_t pts,
    mkvc_submission** out_submission) {
    last_error.clear();
    ...
    *out_submission = nullptr;
    try {
        ...
        std::string error;
        mkvc_result result =
            buffer->implementation->get_view(mutable_view, error);
        if (result != MKVC_OK) return fail(result, std::move(error));
        ...
        *out_submission = handle.release();
        ...
        return fail(MKVC_ERROR_INTERNAL,
                    "unknown native CPU buffer submission failure");
    }
}

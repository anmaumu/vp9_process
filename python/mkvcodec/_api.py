...
class GpuResourcePoolStats:
    """Current occupancy of a fixed-capacity external GPU resource pool."""
    capacity: int
    in_use: int
    peak_in_use: int
    acquisitions: int
...
class IntelUsmPoolSlot:
    """Exclusive writable lease over one caller-preallocated USM resource."""
    def __init__(
...
class IntelUsmFramePool:
    ...
    def try_acquire_slot(self) -> IntelUsmPoolSlot | None:
        """Reserve immediately, or return ``None`` when every slot is leased."""
    ...
    def try_acquire(
        ...
        """Acquire immediately or return ``None`` without accepting ownership."""

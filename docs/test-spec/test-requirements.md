...
- `--reuse-program`はbatch単位のtest-only context/program/kernel再利用比較。`MKVC_OPENCL_REUSE_PROGRAM=1`時は命令割り当てを2個、既定0では2×frames個として解析する。`mkvc_opencl_reuse_session`で明示close/逆順解放/display anchor保持/二重close/途中失敗/再入・busy close/別display・device拒否をGPUなしで検証する。`mkvc_intel_opencl_reuse_av1_roundtrip`は32-frame画素/PTS/owner解放とbuild回数1を実機検証する。long soakではbatchごとにsessionを破棄し、再利用で画像ownerの保持が無制限にならないことを確認する。MAP要求減少をkernel copy減少と同一視しない。

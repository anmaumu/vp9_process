...
| ID | Test requirement | Level | Environment |
|---|---|---|---|
| `TEST-DEC-001` | Python/.NET Capture既定autoと動画情報属性、read/read_bgr/read_nv12/read_surfaceの形式とEOS、CPU VP9 threads=0の1..16 bounded自動解決と明示値保持、conversion_threads 0/1/2/4の画素一致と範囲外拒否 | unit/integration | backend CI |
...
| `TEST-CPUINT-006` | GPU decode→NumPyは`cpu_readback`、CPU borrowed共有は`zero_copy`、owned/非表現strideは`cpu_normalization`、BGR/NV12等は`pixel_conversion`、GPU surface往復は`shared_surface+zero_copy`となることをC/C++/Python/.NET snapshotで検査する。`require_gpu_resident`時のCPU read/writeはbackend処理前に拒否されcounterを増やさず、`driver_internal_observed=0`をdriver zero-copy証明として扱わない | trace/integration/hardware | CPU/Intel/NVIDIA |
...
| `TEST-PY-002` | Pythonのapi/native/interop/internal階層、root公開名限定、high-levelからnative package境界を通る依存方向、旧private import互換性を検証 | source/import/integration | Python CI |
...
| ID | Test requirement | Level | Environment |
|---|---|---|---|
| `TEST-PKG-001` | Windows PE32/PE32+の通常/delay-load importを再帰走査して全非system DLLの解決と到達性を検証し、wheel/NuGet/zipを展開して主Coreと全native依存、LICENSE/PATENTS/NOTICE/SBOMを確認し、隔離環境でbindingからloadする。malformed PE、未解決import、余分な依存をfail closedで拒否する | release | packaging CI |

...
managed worker thread. It supports deadlines and cancellation of the managed
wait while leaving native encode ownership unchanged. A SafeHandle dangerous
reference spans the entire asynchronous operation, so concurrent submission
Dispose cannot free the native completion object. The smoke test forces pending
work and covers success, timeout, cancellation and Dispose/wait races.
...
`implementation-history.md`. A concise `implementation-status.md` now reports
the current qualification suites, capability boundaries, memory model, public
API stability and remaining acceptance work. Docgen copies both sources, MkDocs
strict navigation exposes both pages, and the generated specification overview
rewrites both relative links. No historical checkpoint or measurement was
discarded.
...
| `EXT-CS-001/002`, `INT-CS-001`, `TEST-CS-001` | .NET 8 P/Invoke types, typed exception, CPU/GPU frame SafeHandle ownership, IDisposable writer/capture, all 8-bit CPU formats, native-pool submission `WaitAsync`, leased `ReadSurface`, `WriteSurface`, descriptor/native-handle/wait access and strict GPU copy policy | `mkvc_dotnet_build`, `mkvc_dotnet_smoke`; GPU descriptor/native-handle/copy-policy ABI sizes asserted as 136/64/20 bytes; project-local official .NET SDK 8.0.415 performs warning-free SDK-style build and loads the Windows native DLL for CPU format and async completion round-trips | Windows and Linux CPU managed smoke pass. GPU source API builds; Intel/NVIDIA managed hardware transcode smoke and NuGet publication remain pending |
...
Repository Python checks install NumPy explicitly. The two Linux `LD_AUDIT`

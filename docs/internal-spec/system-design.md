...
- `INT-GPU-007`: Intel external consumerとの同期はD3D11 fence/keyed mutexまたはoneVPL/VA completionを明示し、暗黙の同時accessを許可しない。
  VA native同期が保証するのはそのsurfaceへVAで投入済みの処理だけとする。importから最終consumer完了まで外部書込みを禁止し、OpenCL/SYCL等で行った書込みは外部APIの同期完了または汎用producer queryで別途保証する。Python VA importはstable-ABI native owner holderを用い、import失敗時はholderだけをcancelして利用者ownerの所有権を受け取らない。.NETは同じ契約でGCHandleを解除する。Windows D3D11 native fence adapterは後続実装とする。

...
- `EXT-GPU-009`: CUDA pointer/CUarray、D3D11 texture、VA surface、対応時USM/DLPackのimport APIはresource owner、layout、device/context、producer completion、release callbackを受け取る。
  ...
  Python `IntelUsmFramePool`は`(pointer, owner)`を固定slotとして登録し、`acquire_slot()`で外部producer用の排他的resourceを返す。`slot.import_frame()`はreservationをUSM frame ownerへ移し、frame、DLPack、VA/encoderへ引き継いだownerの全leaseが終わるまで再取得させない。`try_acquire_slot()`/`try_acquire()`は所有権を受け取らず`None`を返す。C++/.NETは同じC ABI reservationをRAII/SafeHandleで公開する。

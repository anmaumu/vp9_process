...
- `EXT-GPU-008`: PythonではIntel USM対応経路およびNVIDIA CUDA対応経路をDLPack protocolで受け渡しでき、consumer指定streamへ正しいdependencyを設定する。Intel linear device-USMは同期済み指定またはborrowed Level Zero eventを受け付ける。eventはpublic `zeEventQueryStatus`だけでpollし、reset/destroyしない。event付きUSMへ同期的な`dependency_registrar(event, consumer_stream)`を指定した場合、DLPack exportはregistrarの成功後にhost待機せず返す。registrar未指定、consumer stream未指定またはeventなしの場合は安全側のhost待機を維持する。registrarはconsumer runtimeのqueue ABIとevent importを所有し、callback終了後にevent/streamを参照保持してはならない。
- `EXT-GPU-009`: CUDA pointer/CUarray、D3D11 texture、VA surface、対応時USM/DLPackのimport APIはresource owner、layout、device/context、producer completion、release callbackを受け取る。
  caller-preallocated GPU resource向けに固定容量のversioned reservation poolを提供する。allocationはCUDA/oneAPI/D3D11/VA adapter側でpool作成前に行い、native libraryはvendor allocator callbackやSYCL C++ ABIを要求しない。zero-timeout acquireは満杯時に`WOULD_BLOCK`、有限timeoutは`TIMEOUT`、解放待ちはslot単位generationで保護する。pool ownerを先に破棄しても既存reservationは有効とする。capacity/in-use/peak/acquisition/rejection/wait時間を照会できる。
  Python `IntelUsmFramePool`は`(pointer, owner)`を固定slotとして登録し、`acquire_slot()`で外部producer用の排他的resourceを返す。slotは取得直後だけ書込み可能であり、`slot.import_frame()`による所有権移譲または`slot.close()`による返却のどちらか一度だけを受理する。移譲後の再importと、返却後のimportは`RuntimeError`で拒否する。移譲時はreservationをUSM frame ownerへ移し、frame、DLPack、VA/encoderへ引き継いだownerの全leaseが終わるまで再取得させない。`try_acquire_slot()`/`try_acquire()`は所有権を受け取らず`None`を返す。C++/.NETは同じC ABI reservationをRAII/SafeHandleで公開する。

...
- `EXT-GPU-008`: PythonではIntel USM対応経路およびNVIDIA CUDA対応経路をDLPack protocolで受け渡しでき、consumer指定streamへ正しいdependencyを設定する。Intel linear device-USMは同期済み指定またはborrowed Level Zero eventを受け付ける。eventはpublic `zeEventQueryStatus`だけでpollし、reset/destroyしない。初期sliceのDLPack取得はevent完了をhost待機するため、任意consumer SYCL queueへ直接dependencyを挿入する完全非同期経路とは区別する。

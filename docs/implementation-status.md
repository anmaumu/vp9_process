...
thread-safe producer/release callbacks. Python owner adapter, CUarray, DLPack
import, and CUDA-event dependency insertion remain pending.
...
| `EXT-GPU-004..005`, `INT-GPU-008/010` external-frame subset | `mkvc_gpu_frame_import_external`, C++ RAII factory and .NET managed-owner adapter; producer query, immutable descriptor/native handle, single-shot release and CUDA-pointer backend resource binding | native/C++/.NET pending→complete, descriptor, invalid-layout and exactly-once release tests | C ABI/C++/.NET CUDA-pointer foundation complete; Python adapter, DLPack/CUarray and Intel oneVPL resource import pending |
...
| `EXT-GPU-008`, `INT-GPU-010/011`, `TEST-GPU-009/010` DLPack foundation | C ABI exports each linear CUDA NV12 plane as a standard uint8 `DLManagedTensor`; native deleter retains the GPU-frame lease independently of Python GC; CPython stable-ABI extension creates protocol capsules whose destructor handles only unconsumed tensors; `GpuFrame.plane()` exposes `__dlpack__`/`__dlpack_device__`; texture/VA/nonlinear and unsupported event dependencies fail closed | native ABI test validates Y/UV pointer offset, shape, byte strides, CUDA device identity and source-release-before-consumer-deleter ordering; extension test validates unconsumed destruction and consumed-capsule no-double-delete | Native and Python source paths complete. CuPy hardware qualification, consumer-stream CUDA event insertion, subprocess shutdown stress, built-extension wheel inspection and Intel USM remain pending |

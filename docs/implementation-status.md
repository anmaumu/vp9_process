...
|---|---|---|---|
| `EXT-CODEC-001` / `AC-CODEC-001` | libvpx VP9 CPU encode | `mkvc_cpu_vp9_encode` | encode complete; decode pending |
| `EXT-CONT-001..003` / `AC-CONT-001` | libwebm WebM mux/finalize | `mkvc_cpu_vp9_external_decode`, `mkvc_cpu_vp9_metadata` | WebM VP9 writer complete; MKV/decode pending |
...
| `INT-CPU-001` | libvpx VP9 encode | Linux GCC build and encode | encode complete |
| `INT-CONT-001/003` | libwebm mux, `V_VP9`, PTS/duration/keyframe | FFmpeg/ffprobe tests | writer subset complete |
...
| `TEST-CODEC-001` | VP9 encode/decode round-trip with quality metrics | external decode only | PSNR/SSIM and internal decoder pending |

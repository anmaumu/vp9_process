...
| `EXT-ENC-013`, `EXT-FRAME-010..012` native-pool subset | fixed-capacity C ABI pool, generation-checked slot lease, nonblocking/timed backpressure, writable Python NumPy views, async encoder ownership transfer | `mkvc_cpu_frame_pool`, `mkvc_python_roundtrip` capacity/generation/view-lifetime/round-trip coverage | native allocation slice complete; OS page-lock, .NET pool, strict fallback and detailed copy trace remain pending |

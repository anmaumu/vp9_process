...
## 2026-09-04: Synchronized Intel device-USM DLPack adapter

The first versioned Intel USM slice is now public through the existing C ABI frame
descriptor and Python `GpuFrame.import_usm_nv12()`. It accepts only positive/even
linear NV12 device-USM layouts with nonzero pointer, SYCL context and queue,
consistent caller-declared device/pitch metadata, and an owner lease. Until a
native SYCL-event completion adapter is added, the
caller must finish the producer queue and explicitly declare it synchronized;
callback/pending USM imports fail closed.
...
allocation releases. Native and Python negative/lifetime tests pass on Windows and
Linux. Native SYCL event dependency, allocator pool/backpressure, non-linear and
...
| `EXT-GPU-008`, `INT-GPU-010/011`, `TEST-GPU-009/010` DLPack foundation | C ABI exports linear CUDA and synchronized Intel device-USM NV12 planes as standard uint8 `DLManagedTensor`; native deleter retains the GPU-frame lease independently of Python GC; Python imports CUDA pointer/DLPack and explicit synchronized USM with owner/context/queue/layout validation | native ABI/Python lifetime and negative tests; CuPy 14.2 on RTX 2060; dpnp 0.20/dpctl 0.22.1 on Arc B580 verifies kDLOneAPI pointer identity and public USM→VA→oneVPL AV1 roundtrip | CUDA path and synchronized linear Intel USM slice complete. Native SYCL-event dependency, USM pool/backpressure/fault/soak, direct oneVPL USM consumption, processed-resource NVENC positive encode and built-wheel inspection remain pending |

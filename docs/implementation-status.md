...
| `EXT-CS-001/002`, `INT-CS-001`, `TEST-CS-001` | .NET 8 P/Invoke types, typed exception, CPU/GPU frame SafeHandle ownership, IDisposable writer/capture, owned I420 result, leased `ReadSurface`, `WriteSurface`, descriptor/native-handle/wait access and strict GPU copy policy | `mkvc_dotnet_build`, `mkvc_dotnet_smoke`; GPU descriptor/native-handle/copy-policy ABI sizes asserted as 136/64/20 bytes; Windows Roslyn compiles the binding and smoke sources directly against the .NET 8 runtime assemblies | Linux CPU ABI/native-load and bounded writer → prefetch capture 10-frame ordered round-trip previously passed. GPU source API compiles; current hosts lack a .NET SDK, so SDK-style build plus Intel/NVIDIA managed hardware transcode smoke remain pending. Packed CPU formats, async Task API and NuGet publication are also pending |
